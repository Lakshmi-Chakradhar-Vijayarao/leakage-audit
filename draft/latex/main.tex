...
running near the ceiling. Fitting both axes at once on a purpose-built
$K\times$AUROC$_0$ factorial qualifies the first of the two: $\ln K$ alone
explains only $R^2=0.114$ across that grid, the $R^2=0.970$ logarithmic law
holds \emph{within} a fixed operating point rather than in general, and the
surface is described well only by a multiplicative empirical fit,
$\text{gap}\propto K^{0.30}e^{-3.12\,\Phi^{-1}(\text{AUROC}_0)}$
($R^2=0.94$) --- an empirical fit, explicitly not a bound.
...
\item \textbf{The severity relationship:} evidence that inflation in this
   ...
   rather than by which of the five mechanisms is responsible (§5) --- plus
   a $K\times$AUROC$_0$ factorial that fits both axes jointly, shows the two
   interact multiplicatively rather than additively, and thereby limits the
   $K$-law to within-operating-point scope.
...
primary severity harness --- drove the synthetic data draw, the outer
train/test split and the inner cross-validation fold assignment from one
shared \texttt{seed} variable. Across the $100$ replicates those three
sources of variation were therefore perfectly confounded: no replicate
shares a dataset with a different split, or a split with a different fold
assignment. This is exactly the confound \texttt{code/47}'s explicitly
...
reproduces \texttt{code/47}'s independently-written default cell
($+0.0036$, $p=0.0012$), which is the identical configuration and had used
decoupled seeds all along; under the coupled scheme the two differed. That
...
\subsection{Case Study 4 (severity $\Delta_{\text{wc}}=+0.0054$ AUROC, CI $[+0.0032,+0.0100]$): Test-set-driven best-layer selection — quantized-LLM paper}
...
\texttt{best\_layer} by the argmax, over $33$ layers, of each layer's \textbf{test-set}
AUROC, then reports that same test-set number as the headline result,
with no correction for having tried $33$ hypotheses against the one test
...
estimate is mean $\Delta_{\text{wc}}=+0.0054$ AUROC across all 24 combinations (BCa 95\% CI
...
severity estimate in this paper; that is fixed here. \textbf{Power, stated
rather than left implicit:} the $24$ per-cell estimates have SD $0.0079$, so
this design's minimum detectable effect at $\alpha=0.05$, $80\%$ power is
$+0.0045$ --- below the observed $+0.0054$, but only just, and the
non-saturated half ($n=12$, SD $0.0110$, MDE $+0.0089$) is \emph{under}powered
for its own observed $+0.0065$. The pooled estimate is adequately powered;
the subgroup split below is descriptive, and we do not lean on the
non-saturated cell's individual $p=0.047$.
...
issues 12 and 14). Mechanism 4's is $\Delta_{\text{wc}}=+0.0054$ across 24
published cells ($+0.0065$ on the non-saturated half). Mechanism 5's effect on
...
\item \textbf{Candidate count.} Sweeping $K\in\{1,\ldots,225\}$ moves the
  ...
  falsified on this data once fit as stated (Appendix A). Note also that
  this sweep holds AUROC$_0$ at $0.80$, and the joint fit below shows the
  law does not survive being asked to hold across operating points.
...
$K\in\{3,15,45,135\}\times\text{AUROC}_0\in\{0.70,0.80,0.90,0.95,0.985\}$
...
\begin{tabular}{lccccc}
\toprule
AUROC$_0$ & $K=3$ & $K=15$ & $K=45$ & $K=135$ & span across $K$ \\
\midrule
0.70  & $+0.00177$ & $+0.00604$ & $+0.00932$ & $+0.00922$ & $5.3\times$ \\
0.80  & $+0.00071$ & $+0.00117$ & $+0.00365$ & $+0.00366$ & $5.1\times$ \\
0.90  & $+0.00011$ & $+0.00015$ & $+0.00109$ & $+0.00096$ & $10.3\times$ \\
...
$\text{gap}=a+b\ln K$ alone, $R^2=0.114$ (LOO $-0.072$); $a+cz_0$ alone,
$R^2=0.499$ (LOO $0.352$); the additive $a+b\ln K+cz_0$, $R^2=0.614$ (LOO
$0.438$); with an interaction term, $R^2=0.718$ (LOO $0.553$), and the
interaction is significant ($F(1,16)=5.92$, $p=0.027$); and a multiplicative
form $\text{gap}=\exp(a+b\ln K+cz_0)$, i.e.\ $\text{gap}\propto
K^{0.30}e^{-3.12 z_0}$, $R^2=\mathbf{0.940}$ (LOO $0.642$). \textbf{So a
single joint model does describe this surface well, but only a
multiplicative one:} additivity in $\ln K$ and $z_0$ is rejected, because
$K$'s effect depends on where on the difficulty curve the pipeline sits.
...
as a whole, $\ln K$ \emph{alone} explains almost none of the variance
($R^2=0.114$, and it fails to beat predicting the mean out-of-sample). The
$K$-law is therefore real \emph{within} an operating point and not a
description of severity in general: the table shows why, with a $5$--$10\times$
span across $K$ where there is headroom and essentially no movement at all
by $\text{AUROC}_0=0.985$, where every cell sits between $+0.00000$ and
$+0.00019$ regardless of how many candidates were tried. Read together with
the $48.6\times$ operating-point decline, the practical reading is that
candidate count is a second-order modifier of a first-order operating-point
effect, not a co-equal axis.
...
$n_{\text{val}}$; its best leave-one-cell-out $R^2$ is $0.64$, so even
within its own grid it predicts individual cells only moderately well; and
it is an average, not an upper bound, so it cannot be used to certify that
any particular pipeline's inflation is below anything. Predicting the five
Sweep A cells at $K\in\{5,10,25,75,225\}$ that are not in the grid gives
$R^2=0.853$ (additive), $0.795$ (interaction) and $0.591$ (multiplicative)
--- which tests generalization across $K$, and not across data, since those
cells reuse the same seed streams. We report the fit because the review was
right that the attempt was missing, and we name a formally-derived bound as
future work rather than claim this is one.
...
what would be needed --- it establishes that the two quantities interact,
which a mechanism-level comparison would have to hold constant --- but it
does not close it, and it varies $n_{\text{val}}$ not at all.
...
never call it $\phi$. Two obstacles to the derivation are specific rather
than generic, and either would have to be explained rather than assumed
...
would predict (§5, Sweep D), and the extreme-value functional form the
derivation would most naturally rest on is falsified here once fit as
stated (Appendix A). A theoretically-justified upper bound consistent with
both facts is future work we name rather than claim.
...
4 tested capacities and at 1 of 4 after Holm-Bonferroni; Mechanism 4's $+0.0054$ (BCa 95\% CI
...
than any difference we measure between mechanisms --- but a
$K\times$AUROC$_0$ factorial run to fit the two jointly shows they are not
co-equal: across that grid $\ln K$ alone explains $R^2=0.114$, the two
axes interact ($p=0.027$), and only a multiplicative empirical fit
describes the surface ($R^2=0.94$). The $K$-law is a within-operating-point
regularity, and candidate count is a second-order modifier of a
first-order operating-point effect. We report a
...
\textbf{A third, independent way of seeing the same failure, which does not
depend on any fitted coefficient: the dynamic range is wrong.} Across
$K=3\to225$ the measured gap grows by a factor of $6.10\times$
($+0.00086\to+0.00522$). The $\sqrt{2\ln K}$ factor grows by only
$3.29/1.48=2.22\times$ over that same range. So even in the flattering
constant-$\sigma$ version --- where the fitted $c$ is free to absorb any
overall scale --- the functional form cannot reproduce how much the gap
actually moves; it is off by nearly a factor of three in \emph{span}, not
merely in fit quality. Under the theory's own per-cell $\sigma$ the mismatch
is worse still, because $\sigma_{\text{val}}$ falls $2.66\times$ over the same
range and so pushes the prediction in the opposite direction. This is why we
describe the extreme-value form as falsified rather than as imprecisely
calibrated.
...
\textbf{Correction 3 (a further independent review): Sweep B was
...
simply does not follow it, the same failure of the EVT channel that
Correction 2 documents for the $K$ sweep. What the gap does track is the
...
form} is falsified by this data once fit as stated, and the isolated
$n_{\text{val}}$ dependence, once measured without the sample-size confound,
is cleanly monotone in the direction opposite to the theory's. We therefore
