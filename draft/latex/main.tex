...
layer selection, $+0.0009$ to $+0.0034$ for per-fold checkpoint selection on
the primary synthetic sweep (rising to $+0.0083$ once a downstream
...
\subsection{Case Study 3 (severity small: $\Delta_{\text{synth}}=+0.0009$ to $+0.0034$ AUROC on the primary synthetic sweep): Per-fold checkpoint-selection leakage — MultiHaluDet}
...
capacity, by roughly $2.5$--$4.7\times$: $+0.0042$/$+0.0068$/$+0.0087$/$+0.0083$
un-averaged at capacities 16/48/128/384, against
$+0.0009$/$+0.0017$/$+0.0034$/$+0.0027$ averaged --- and it is significant at
all four capacities un-averaged, against two of four averaged. Reading the OOF
matrix directly agrees ($+0.0060$/$+0.0084$/$+0.0083$/$+0.0040$, significant
...
($0.495$--$0.515$ for every condition and capacity), i.e.\ the fold-models'
...
against \texttt{code/02d}'s $+0.0034$ ($p=0.008$). That difference is
...
severity is $\Delta_{\text{synth}}=+0.0009$ to $+0.0034$ on the isotropic
...
$+0.034$ in F1. That is a spread of roughly $0.000$ to $0.034$ AUROC
...
the gap by $2.5$--$4.7\times$, so this paper's primary estimate is an estimate
...
$50$ randomized splits; Mechanism 3's checkpoint-selection severity $+0.0009$
...
raises the LEAKY$-$CLEAN\_MATCHED gap by $2.5$--$4.7\times$ at every capacity
