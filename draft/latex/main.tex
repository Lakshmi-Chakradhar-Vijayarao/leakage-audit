...
code-verified AUROC-scale severity estimate produces inflation in a narrow
band of roughly $0.000$ to $0.034$ AUROC. What severity \emph{does} track,
strongly and mechanism-independently, is two continuous quantities: the
number of candidates selected among, and the operating point the pipeline
runs at. Holding the leakage mechanism fixed and varying only the task's
calibrated difficulty moves the inflation by a factor of $48.6\times$ ---
from $+0.0093$ at AUROC$_0=0.70$ to $+0.0002$ at $0.985$. That relationship,
previously buried in an appendix, is promoted here to a headline finding
(§5) --- but promoted with a limit an earlier version of this paper asserted
its way past. We previously said the relationship moves severity ``more than
any difference we measure between mechanisms.'' §5 now tests that against our
own shipped data and it does not hold: at achieved operating points within
$0.003$ AUROC of each other, two real-feature harnesses exceed what the
relationship predicts by $14\times$ and $52\times$. The operating point is a
strong severity modifier \emph{within} a harness --- enough to make matching
it a precondition for comparing any two severity numbers, which is how this
paper uses it --- and it is not a law that transports across harnesses or
mechanisms.
...
\textbf{Organization.} §2 places this work against existing leakage
taxonomies and fixes the definition of leakage we use. §3 states how the
case studies were selected and graded for verifiability. §4 presents the
five mechanisms and their measured severities. §5 is the paper's central
result: what actually governs those severities, and the limits of that
finding. §6 gives the checklist and a negative automation result, §7
discusses limitations, and Appendix A documents the correction history
behind §4.3's numbers.
...
guarantees); that work treats the evaluation protocol as given, whereas our
contribution sits upstream of it --- an inflated AUROC corrupts any
downstream calibration analysis built on it. Closer to our concern, PARALLAX
(arXiv 2605.17028) audits 22 detection methods across 12 models and 6
corpora and finds four of six corpora leak the ground-truth answer directly
into the input prompt; once controlled for, most baselines fall to near
chance. That is independent, larger-scale evidence that the field's reported
AUROCs are frequently protocol or construction artifacts, though PARALLAX's
mechanism (benchmark construction) is distinct from all five documented here
(each of which concerns a downstream selection step). Separately,
surface-form correctness metrics (ROUGE-L, exact match) are known to diverge
from LLM-judge or human factual-correctness judgments -- a related but
distinct label-quality concern this paper does not address. Fixing the
leakage patterns we identify does not, by itself, address label-quality or
...
\textbf{What ``severity'' means here, and why the numbers below are not all
on one scale.} An earlier version of this paper used the single word
``severity'' for several quantities that are not mutually comparable in
absolute magnitude, which invites a reader to rank numbers that should not
be ranked. We name them, and every reported severity below carries a tag
--- e.g.\ ``(in $\Delta_{\text{synth}}$ units)'' --- identifying which it is.
...
\item $\Delta_{\text{synth}}$ --- the \textbf{calibrated synthetic-harness
  ...
  Both arms are budget- and seed-matched, and the operating point is the
  same in every cell being compared. Used in §4.3's capacity table, the
  anisotropic and eigenspectrum sweeps, and every cell of §5's sweeps.
...
not a \emph{reference}. When this paper says severities ``cluster'' in a band
of roughly $0.000$ to $0.034$ (§5, Conclusion), that is a statement about the
numerical range these quantities occupy, explicitly \emph{not} a claim that
they are interchangeable measurements of one underlying scale. Mechanism 5's
F1 and accuracy gaps are on a different metric entirely and are never placed
in that band. And because §5 shows the gap falls by $48.6\times$ across the
operating-point range, a $\Delta_{\text{real}}$ and a $\Delta_{\text{synth}}$
measured at different operating points are not comparable in magnitude even
though both are AUROC differences.
...
\textbf{Full 32-layer decomposition.} GUARDIAN's own raw hidden-state cache
contains the complete (700, 32, 4096) Mistral-7B representations -- all 700
samples (the 400-sample CV pool and the 300-sample held-out set) at full
dimensionality, every layer, not just L11. (An earlier version of this
section disclosed this as unavailable; that was incorrect.)
\texttt{code/48\_case\_study\_2\_layer\_decomposition.py} repeats the
...
$\Delta_{\text{sel}}$: gap-at-the-selected-layer minus the mean gap across
all 32 layers, evaluated at whichever layer that split's own CV-argmax
actually selects (a second review caught that an earlier version hardcoded
L11 for every check, including randomized reps that select a different
layer -- fixed to use each split's own selection).
...
$-0.004$ -- indistinguishable from zero, meaning the 18.8-point gap is not
disproportionately concentrated at the selected layer at all once the
population-difference confound is present in every layer equally. Reversing
which half is used for selection (\texttt{H[400:]} selects,
\texttt{H[:400]} held out) selects a different layer entirely (L17, not L11)
and gives a component of $+0.074$ -- a large swing in both which layer is
picked and what the component measures, itself direct evidence that the
sequential-split number tracks an artifact of split order rather than a
stable property of the selection procedure. Recomputing under randomized
stratified $400/300$ splits (never touching the sequential order), each rep
...
specifically selection-attributable penalty from CV-based
...
\emph{A second, smaller asymmetry inside CLEAN\_MATCHED, disclosed at a
review's request.} CLEAN\_MATCHED's epoch count is \emph{selected} by a model
...
--- exact per-seed MDEs at $\alpha=0.05$, $80\%$ power, $n=100$ are $0.0023$
at capacity 48 against an observed $+0.0011$, and $0.0039$ at capacity 384
against an observed $+0.0024$, so both observed gaps sit below the smallest
effect this design could reliably detect. This correction is applied within
this four-capacity family only, not jointly across every p-value in this
section.
...
so the comparison below is checkable rather than asserted). The table above is
the rerun.
...
away?} The harness above --- like MultiHaluDet's own
...
readings. The \emph{averaged} number is the correct estimate of inflation in
what the audited pipeline actually reports, because averaging is what that
pipeline does --- so the table above stands unchanged as this paper's primary
...
\subsubsection{Does the estimate depend on the generative process?}

\textbf{A second, structurally different generative process -- with two
disclosed confounds of its own.} The sweep above draws each class from an
isotropic Gaussian, so a toy-model artifact is a live alternative explanation.
...
covariance and mean-difference direction from the real Mistral-7B/HaluEval
features used below, then rescale the mean difference (keeping the real,
...
selection, the same sign anomaly the epoch-count diagnostic (Appendix A) was
built to explain and which does not recur under either corrected real-feature
...
The comparison above cannot say, because the two sweeps differ in both. An
...
\texttt{sanity\_checks.py}, whose bias correction assumes $\Sigma=I$ --- see
Appendix A, issue 11). Cells: power-law spectra $\lambda_i\propto i^{-\beta}$
...
LEAKY/CLEAN\_MATCHED/PLACEBO architecture used above gives LEAKY beating
...
measurement instrument committing the very pattern it audits for. Appendix A
(issues 5, 6, 8, 9, 13) records the full progression; two findings matter to a
...
this test's history, the calibration actually reaches its intended operating
point rather than saturating far above it. Under the downstream 5-fold
OOF-plus-meta-learner pipeline the operating point falls to
$\approx0.94$-$0.95$ (LEAKY $0.9403$ at capacity 128, $0.9460$ at 384), down
from the $\approx0.96$-$0.97$ the label-conditional calibration produced and
much further from the $0.985$ ceiling the original version ran at --- but still
above the $0.80$ the calibration search achieves under plain regularized
logistic regression, because the MLP-plus-meta-learner stack recovers
discriminative structure in directions other than the single axis the
calibration operates on. A direct severity-magnitude comparison against the
synthetic sweep's $0.80$ harness therefore remains invalid by this paper's own
stated principle, and we do not make one. At its own achieved operating point
the test gives (in $\Delta_{\text{real}}$ units): LEAKY beats CLEAN\_MATCHED by
$+0.0093$ (BCa 95\% CI $[+0.0060,+0.0134]$, capacity 128) and $+0.0077$ (CI
...
permutation $p<0.0001$ at both capacities), which now agree where previously
they did not. CLEAN\_MATCHED beats PLACEBO decisively at both capacities
($+0.0151$, $p=9.4\times10^{-7}$; $+0.0091$, $p=1.0\times10^{-5}$).
...
A separate FP16-vs-AWQ control on the same 50 samples finds identical AUROC
(0.9600 both) at a single point estimate; a resampled version (Appendix A)
gives a 95\% CI of $[-0.061, +0.080]$, consistent with no quantization confound
rather than a demonstration that rules one out at the resolution this severity
estimate needs. This paper's primary severity estimate for Mechanism 3 still
rests on the two synthetic sweeps, which generate fresh samples from a
fully-specified generative process each seed rather than rescaling a small,
fixed real sample, and so share neither the analytic-bias nor the split-leakage
problem found here.
...
versus \texttt{tr\_idx}, Appendix A item 10). \textbf{We do not treat the depth
...
exactly the degenerate control Appendix A item 10's second fix attempt had to
back out of. But a reader should know the two arms are not matched on it, and
...
early-stopping and checkpoint mechanics already ported (Appendix A, issue 14).
...
\texttt{code/49}'s module docstring; Appendix A (issue 12) reports the
factorial $2\times2$ ablation separating this extension's two most recent
corrections.
...
against the one test set. The number of probed layers is model-dependent and
not constant across the 24 cells, as an earlier version of this section
incorrectly implied: it is $33$ for the eight \texttt{llama3.1-8b} cells and
the eight \texttt{mistral-7b} cells, and $29$ for the eight
\texttt{qwen2.5-7b} cells. \texttt{code/45} asserts these counts against the
shipped files so the figure cannot drift. This is the classical ``winner's
curse'' of multiple-hypothesis selection using the exact metric later
reported.

Re-quantifying this mechanism's severity does not require their exact
quantized checkpoint and dataset re-run from scratch, as we first believed:
their repo ships per-layer, per-seed AUROC arrays for all 24
model/dataset/probe-type combinations tested
(\texttt{code/external/HallucinationPatternDetection/results/probes/*.json};
3 seeds each). For each combination
...
paper's other severity estimates rely on (binormal identity, Bayes-optimal
AUROC target), with a simple logistic-regression classifier in place of the
full SweepMLP-plus-meta-learner architecture --- threshold-selection leakage
does not depend on which classifier produced the probabilities, so a simpler
classifier keeps this demonstration self-contained
(\texttt{code/46\_mechanism5\_threshold\_selection.py}). \textbf{Correction:}
an earlier version of this generator itself committed the exact
calibration-inversion class of error Appendix A (issue 11) documents finding
elsewhere in this paper's own tooling --- the class-mean offset used the full
separation rather than half of it on each side, silently quadrupling the
realized Fisher $J$ relative to the labeled operating point. It was caught by a
subsequent independent review and fixed, verified via a dedicated,
non-tautological calibration guardrail (\texttt{code/sanity\_checks.py}).
Comparing a threshold selected on the test labels
(\texttt{find\_best\_thresholds}, verbatim ported from MultiHaluDet's own code)
against a threshold selected on an independent validation split never touching
the test labels, across five operating points spanning this paper's other
severity estimates, under the corrected calibration: F1 gaps range from
$+0.0212$ ($0.985$, MultiHaluDet's own reported regime) to $+0.0338$ (AUROC
target $0.80$), with no clean monotonic trend across operating points
($0.70{\to}0.0228$, $0.80{\to}0.0338$, $0.90{\to}0.0311$, $0.95{\to}0.0233$,
$0.985{\to}0.0212$), all significant at $p<10^{-31}$ over $200$ seeds (BCa 95\%
CIs excluding zero throughout); accuracy gaps are of comparable magnitude
($+0.0215$ to $+0.0457$).
...
F1 gap reported above is also structurally guaranteed non-negative: the LEAKY
threshold is chosen by an argmax over the very same grid the test F1 is then
scored on, so it cannot do worse than any other threshold drawn from that grid,
including HONEST's. An earlier draft compared these F1/accuracy gaps directly
against Mechanism 3's AUROC gaps and described Mechanism 5 as ``an order of
magnitude more severe'' -- that comparison conflated two different metrics
measuring two different things and has been removed. The honest statement is
narrower: Mechanism 5 leaves the pipeline's reported AUROC entirely unaffected,
but materially inflates every threshold-dependent metric (F1, accuracy, MCC,
Cohen's $\kappa$, and any ECE computed from those threshold-derived
predictions) that MultiHaluDet also reports at that same selected threshold ---
a distinct, real leak in its own right, not a directly rankable ``more or less
severe than Mechanism 3.''
...
\emph{only} on (a) --- whether the layer is chosen on a CV statistic or on the
test set. Mechanisms 4 and 5 share (a) and (c) and differ only on (b).
Mechanisms 1 and 3 share (c), the property that makes both feed a downstream
estimator. And the five are not even mutually exclusive in practice:
Mechanisms 3 and 5 co-occur inside a single file of a single audited
repository (\texttt{MultiHaluDet/run\_pipeline.py}), which is how we found
Mechanism 5 at all. There is no partition criterion under which these are five
disjoint kinds. That is not a weakness for the taxonomy's intended use --- the
axes are what tell an auditor \emph{where to look} --- and it is arguably
predicted by the finding this section reports: if severity is governed by
continuous quantities rather than by discrete mechanism identity, one should
expect the mechanisms to be points along shared axes and neighbouring points
to behave similarly.
...
\texttt{code/47\_selection\_multiplicity\_sweep.py} varies, one at a time, the
number of candidates selected among ($K$), the selection-set size
($n_{\text{val}}$), and the model's operating point (the calibrated
Bayes-optimal AUROC), holding the leakage mechanism itself fixed:
...
\item \textbf{Candidate count.} Sweeping $K\in\{1,\ldots,225\}$ moves the
  ...
  \emph{A disclosure about the sweep's own operating point:} the nominal
  target is fixed at $0.80$, but the \emph{achieved} LEAKY mean is not --- it
  runs from $0.6269$ at $K=1$ to $0.7574$ at $K=225$ --- so the full-range fit
  is partly confounded with the operating-point relationship below. Restricted
  to $K\geq10$, where the achieved operating point spans only
  $0.7565$--$0.7616$, the law is essentially unchanged ($b=0.00107$,
...
\item \textbf{Selection-set size --- and a correction to how this sweep was
  labelled.} An earlier version reported an ``$n_{\text{val}}$ sweep'' here
  that was nothing of the kind: it varied $N_{\text{SAMPLES}}\in\{350,700,2800\}$,
  which moves the training set, the test set \emph{and} the per-fold
  validation set together, so its non-monotone pattern ($+0.0026$ at
  $n_{\text{val}}\approx56$, not significant; $+0.0036$ at $\approx112$;
  $+0.0005$ at $\approx448$) could not be attributed to $n_{\text{val}}$ at
  all. It is relabelled a \emph{sample-size} sweep, with that confound stated.
  The isolated version holds $N_{\text{SAMPLES}}=700$ fixed --- and with it the
  outer train/test split and the test set the metric is reported on --- and
  varies only the inner CV fold count $K_{\text{CV}}$, which is what sets
  $n_{\text{val}}=n_{\text{train}}/K_{\text{CV}}$ (\texttt{code/47}, Sweep D).
  It gives a \emph{monotone non-increasing} relationship, in the direction
  \emph{opposite} to the winner's-curse prediction: $+0.0087$ at
  $n_{\text{val}}=280$, $+0.0036$ at $186$, $+0.0036$ at $112$ (the two middle
  cells are tied to within $3\times10^{-5}$, not strictly ordered), and
  $+0.0005$ (not significant, $p=0.22$) at $56$. Larger selection sets give
  \emph{larger} gaps here. The measured $\sigma_{\text{val}}$ does move as the
  theory expects ($0.0331$ at $n_{\text{val}}=280$ rising to $0.0375$ at $56$),
  so it is not that the selection signal fails to get noisier --- the gap
  simply does not follow it. What the gap does follow is the operating point:
  because a fold's train and validation parts partition one fixed pool, smaller
  $n_{\text{val}}$ means a larger per-fold training set (from $280$ to $504$)
  and hence a better model, and the LEAKY operating point rises monotonically
  across exactly these cells ($0.7474\to0.7523\to0.7586\to0.7599$). That
  residual coupling is inherent to cross-validation and we disclose it rather
  than claim the isolation is complete --- but it means this sweep, read
  together with the operating-point sweep above, is further evidence for the
  operating-point account and against the $1/\sqrt{n_{\text{val}}}$ one. We no
  longer describe any $n_{\text{val}}$ relationship as ``directionally
  consistent with the winner's-curse account.''
\end{itemize}
...
not a theorem} (Limitations (2a)). The existing sweeps cannot answer this:
Sweep A varies $K$ at a fixed $\text{AUROC}_0=0.80$ and Sweep C varies
$\text{AUROC}_0$ at a fixed $K=45$, so together they form a \emph{cross}
through the plane, not a grid --- no cell has both coordinates off the
reference, and no cross can exhibit an interaction. \texttt{code/57} runs the
missing cells: a full
...
three arguments, not three --- Sweep D shows $n_{\text{val}}$ is not an
independent third axis in this harness). The $K=45$ column reproduces Sweep C
exactly, to $0$ in all five cells.
...
column carrying the least contrast. Every conclusion below is accordingly
refit five ways --- on all $20$ cells and with each $K$ column deleted in turn
...
$0.948$ versus $0.547$) is a statement about the \emph{scale} on which the two
axes combine, not evidence that they interact. \textbf{We therefore withdraw
the previous version's claim that ``the two axes interact multiplicatively
rather than additively.''} On the corrected grid there is no significant
interaction to report, and the multiplicative fit does not need one. This
conclusion survives the test that killed its predecessor: refitting with each
$K$ column deleted in turn, the interaction term is non-significant every time
($p=0.18$, $0.22$, $0.38$, $0.64$), and the multiplicative fit stays strong
throughout ($R^2=0.967$--$0.994$). Its operating-point coefficient is stable
($c=-2.80$ to $-3.17$); its candidate-count exponent is the least stable
quantity in the fit ($b=0.09$ to $0.21$, lowest when the $K=15$ column is
removed), consistent with $K$ being the weaker axis and with $K=15$ being the
column where the harness has least contrast. We quote $b$ as a range rather
...
description of severity in general: the table shows why, with a $4$--$7\times$
span across $K$ where there is headroom and essentially no movement at all by
$\text{AUROC}_0=0.985$, where every cell sits between $+0.00012$ and $+0.00019$
regardless of whether $15$ or $405$ candidates were tried. This qualification
is \emph{stronger} on the corrected grid than on the degenerate one it replaces
($R^2=0.038$ here against $0.114$ there), so unlike the interaction claim it
does not depend on the cells that were removed. Read together with the
operating-point decline, the practical reading is that candidate count is a
second-order modifier of a first-order operating-point effect, not a co-equal
axis.
...
$n_{\text{val}}$; and it is an average, not an upper bound, so it cannot
certify that any particular pipeline's inflation is below anything. It also
does not transport across harnesses: §5.4 shows two real-feature harnesses
exceeding this surface's own operating-point relationship by $14$--$52\times$
at a matched operating point. We report the fit because the review was right
that the attempt was missing, and we name a formally-derived bound as future
work rather than claim this is one. \textbf{The out-of-sample check is also
...
subtler form, the error this section already corrects. This harness does not
...
explains any \emph{residual} variance once these are controlled. We do not
claim it explains none. That experiment is the obvious next step and is not run
here. The joint grid narrows the gap between what is measured and what would
be needed --- it establishes the \emph{scale} on which the two quantities
combine, which a mechanism-level comparison would have to hold constant --- but
it does not close it, and it varies $n_{\text{val}}$ not at all. \textbf{And
§5.4 sharpens what that missing experiment has to be.} Two of the three
harnesses compared there differ in mechanic as well as in harness, and they
differ from the synthetic surface's prediction by more than an order of
magnitude at a matched operating point. That is not evidence that mechanism
identity matters --- harness and mechanic are confounded in exactly those two
rows --- but it does mean the one-quantity-at-a-time result can no longer be
read as showing that mechanism identity is unimportant \emph{in general}, only
that it is unimportant relative to $K$ and the operating point \emph{within} a
single harness. Disentangling the two is now the highest-value experiment this
line of work has open.
...
found after this scanner was built and is not covered by it) and ran it against
7 repositories: the two already-audited case studies (MultiHaluDet,
HallucinationPatternDetection), to test whether it catches known bugs, plus 5
additional, not-previously-examined, externally-found hidden-state-probing
repositories, to test its false-positive rate on presumed-clean pipelines
(\texttt{code/04\_leakage\_linter.py},
\texttt{results/leakage\_linter\_report.json}). This is a heuristic
line-matcher, not a validated static analyzer; every flag was manually read and
...
(Mechanism 4) --- direct evidence that the two false negatives below are
failures of the \emph{scanner} rather than artifacts of the authors'
...
--- and neither is included in the 7-hit count below, which is the scanner's
output, not a human audit's.
...
This is a prevalence statement about 2 of 7 repos, not a representative sample
-- all 7 were found via targeted search for exactly this failure class, not a
random or exhaustive sample of the field (HaRP and GUARDIAN, this paper's own
pipelines and the source of Case Studies 1-2, were never run through the
scanner at all). The true population-level prevalence of any of these five
mechanisms remains unknown and is not estimated by this count. Building and
validating a better analyzer is follow-on work this paper does not do.
...
both true positives in the two known-buggy repositories, plus the two
additional sites reported above, where the regex scanner found none. That is
...
reproducible from what ships with this paper, on the terms §4 grades them
against; \textbf{Case Study 1 is the exception, and no script, log, or data
file supporting its $+0.19$ figure is included here.} The regex linter in §6
is included in full and is, per its own results, a confirmed non-detector on
this corpus (0 true positives, 2 false negatives on the corpus's own known
bugs) -- included for transparency, not as a working tool a reader should
rely on today.

\textbf{Scope, and what these numbers mean operationally.} This paper makes no
inference-economy claim: it is an evaluation-protocol audit, not an
inference-time method, and fixing any of the five mechanisms makes no claimed
difference to deployment cost. Its value is entirely in correcting reported
detection metrics. A detector deployed at a reported AUROC above its true value
will, at a fixed threshold, pass more hallucinations to production and flag
more correct outputs for unnecessary review than promised --- but the
code-verified inflations measured here are on the order of $0.002$ to $0.034$
AUROC, not the tens of points an earlier framing of this work implied.
Practitioners should read this as: these mechanisms are real, worth fixing
because they are cheap to fix and because they corrupt comparisons between
methods, but not as an argument that the subfield's reported numbers are
broadly wrong by large margins. We lack deployment-volume and review-cost data
for any audited system, so we do not attempt a cost estimate. Where large gaps
\emph{do} appear (Mechanism 5's $+0.021$ to $+0.034$ on F1 and accuracy), they
are on threshold-dependent metrics, not AUROC, and are not directly comparable
to the AUROC-scale numbers.

\textbf{This audit's central lesson recurs in related, independently-conducted
work} on hidden-state signals and benchmark severity estimates in adjacent
areas of LLM evaluation, none sharing a leakage mechanism with the four case
studies here: an initially plausible result substantially weakens or changes
character once a specific confound is directly tested rather than merely
...
optimism, and this is a real gap rather than an omission for space.} A
function $\phi(K, n_{\text{sel}}, \text{AUROC}_0)$ upper-bounding
...
turn §6's checklist into a calculator. §5.5 reports an \emph{empirical joint
fit} instead, which is not a bound and which we never call $\phi$. \emph{One}
obstacle to the derivation is specific rather than generic: this harness's
isolated $n_{\text{val}}$ dependence runs \emph{opposite} to the
$1/\sqrt{n_{\text{sel}}}$ direction such a derivation would predict (§5, Sweep
D). \emph{A second obstacle previously listed here has been withdrawn:} we had
also cited the extreme-value functional form as falsified in this harness, but
that test used a misspecified $\sigma$ and is uninformative in either
direction --- it is a measurement gap, and closing it needs only a harness
...
sampling-noise SD the derivation requires (Appendix A); we withdraw the
previous version's ``falsified'' language, report an empirical logarithmic
regularity instead, and have no mechanistic explanation for it.
...
has limited power (§4.3).
\emph{(4a) The fixed-$d$ eigenspectrum experiment (§4.3) separates covariance
shape from dimensionality but not from operating point}, because anisotropy
lowers the achieved operating point even at a fixed Bayes-optimal $J$ and at
capacity 128 the two are perfectly rank-confounded. Separating them would need
a generator that holds the \emph{achieved} operating point fixed --- a
calibration search per cell rather than a closed-form one.
...
$n_{\text{test}}=2000$ (§4.5).
\emph{(4d) The scanner's true-positive/false-positive classification rests
on two raters, not a validated protocol} (§6): the primary rater is this
paper's author and knew the ground truth, and while a second, blinded rater
agreed $7/7$, Cohen's $\kappa$ is undefined on a single-category marginal and
seven non-independent sites from two repositories is a weak reliability check.
\emph{(5)} We could not re-run either external paper's full 7B-scale pipeline
within this paper's compute budget, so Case Studies 3 and 4's severity for the
\emph{actual published numbers} remain open questions; our reconstruction of
Case Study 3 supports a small, positive estimate at 2 of 4 tested capacities
individually (1 of 4 after Holm-Bonferroni), and MultiHaluDet's actual
architecture (6 transformer layers, multi-scale, heavy augmentation) is more
expressive still, so this should not be over-read in either direction for their
pipeline. The real-feature test (§4.3) is also single-dataset, single-language
($n=400$ HaluEval English \texttt{qa\_samples} only), despite MultiHaluDet's own
contribution being explicitly multilingual; and even leak-free, no calibration
of this feature set's between-class mean, at any strength, brings it onto the
synthetic sweep's exact $0.80$ operating point, because a real portion of its
discriminability lives in within-class structure the calibration method cannot
touch -- so its severity estimate, while now trustworthy, is reported at its
own ($\approx0.94$-$0.95$) operating point rather than the synthetic sweep's,
and the two are not numerically comparable. Whether the synthetic severity
estimate transfers to real hidden states at the \emph{same} operating point --
on this dataset, non-English datasets, or larger ones -- remains untested
rather than confirmed or excluded.
...
Holding the leakage mechanism fixed and varying only the number of candidates
selected among moves the gap monotonically, well fit by $a+b\ln K$
($R^2=0.970$); varying only the operating point moves it by $48.6\times$, from
$+0.0093$ at AUROC$_0=0.70$ to $+0.0002$ at $0.985$ (§5). Both effects are
larger than any difference we measure between mechanisms \emph{inside that
harness} --- though §5.4 shows the operating-point relationship does not
transport outside it, exceeding by $14$--$52\times$ at a matched operating point
in this paper's own real-feature harnesses, so it is a within-harness severity
modifier and not a general law. A $K\times$AUROC$_0$ factorial run to fit the
two jointly shows the axes are not co-equal: across that grid $\ln K$ alone
explains $R^2=0.038$, and a separable multiplicative empirical fit describes the
surface well ($R^2=0.976$), so candidate count is a second-order modifier of a
first-order operating-point effect. Two claims of earlier versions are
withdrawn rather than restated: that the two axes \emph{interact}, which came
entirely from a degenerate $K=3$ column in which the leaky and control arms are
bitwise identical in most folds; and that the extreme-value form
$c\,\sigma_{\text{val}}\sqrt{2\ln K}$ is falsified, which the only $\sigma$ this
harness produces --- a single training trajectory's dispersion --- cannot
establish either way (Appendix A). The empirical $a+b\ln K$ law never
...
measurement instrument all to hold at once --- is the strongest argument in
this paper for the checklist we provide, more so than any single mechanism in
...
this checklist to a fixed, defined sample of 15-20 externally published probing
papers, selected before any of them are examined, is planned as follow-on work;
that protocol is already drafted.
...
\textbf{Smaller checks, retained for completeness.} \emph{Power.} Exact
